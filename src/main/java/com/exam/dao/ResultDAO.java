...
public class ResultDAO {
    ...
    public void saveResult(int studentId, int examId, int score, int totalMarks) throws SQLException {
    ...
    public List<Result> getResultsForStudent(int studentId) throws SQLException {
        List<Result> list = new ArrayList<>();
        String sql = "SELECT r.score, r.total_marks, r.submitted_at, e.title " +
    ...
    public List<Result> getAllResults() throws SQLException {
        List<Result> list = new ArrayList<>();
        String sql = "SELECT r.score, r.total_marks, r.submitted_at, s.name AS student_name, e.title AS exam_title " +
                "FROM results r JOIN students s ON r.student_id = s.id JOIN exams e ON r.exam_id = e.id " +
    ...
    public int countResults() throws SQLException {
        try (Connection conn = DBConnection.getConnection();
             Statement st = conn.createStatement();
             ResultSet rs = st.executeQuery("SELECT COUNT(*) c FROM results")) {
    ...
    public int countStudents() throws SQLException {
        try (Connection conn = DBConnection.getConnection();
             Statement st = conn.createStatement();
             ResultSet rs = st.executeQuery("SELECT COUNT(*) c FROM students")) {
            if (rs.next()) return rs.getInt("c");
        }
        return 0;
    }
